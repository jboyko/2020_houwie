...
generateParameters <- function(continuous_model, minAlpha, maxAlpha, minSigma2, maxSigma2, minTheta, maxTheta, discrete_model, minRate, maxRate){
  ...
  par_alpha <- runif(1, minAlpha, maxAlpha) * seq_len(k.alpha)
  par_sigma <- runif(1, minSigma2, maxSigma2) * seq_len(k.sigma)
  par_theta <- runif(1, minTheta, maxTheta) * seq_len(k.theta)
