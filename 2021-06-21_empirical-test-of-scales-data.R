...
quickRun <- function(index){
  if(index == 1){
    fit.BM1_A <- hOUwie(phy, dat, 2, nSim, index.cor = DiscModelB, model.ou = "BM1", weighted = TRUE)
    return(fit.BM1_A)
  }
  if(index == 2){
    fit.BM1_B <- hOUwie(phy, dat, 1, nSim, model.cor = "ER", model.ou = "BM1", weighted = TRUE)
    return(fit.BM1_B)
  }
  if(index == 3){
    fit.OU1_A <- hOUwie(phy, dat, 2, nSim, index.cor = DiscModelB, model.ou = "OU1", weighted = TRUE)
    return(fit.OU1_A)
  }
  if(index == 4){
    fit.OU1_B <- hOUwie(phy, dat, 1, nSim, model.cor = "ER", model.ou = "OU1", weighted = TRUE)
    return(fit.OU1_B)
  }
  if(index == 5){
