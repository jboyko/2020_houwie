...
getContrasts <- function(phy, dat, alpha){
  ...
  for(W_i in unique(phy$edge[,1])){
    ...
    # calculate contrast ci as a weighted difference between xl and xr
    contrasts[count] <- getC_i(alpha, dec_values[1], dec_values[2], dec_distance[1], dec_distance[2])
    # calculate an intermediate value wi as a weighted average between xl and xr
    weights[count] <- getW_i(alpha, dec_values[1], dec_values[2], dec_distance[1], dec_distance[2])
    ...
    D_i <- getDeltaD(alpha, dec_distance[1], dec_distance[2])
...
for(i in 1:100){
  ...
  PossibleContrasts <- getContrasts(phy, dat, alpha)
  PhyLm[i] <- OU1d.loglik(dat, phy, "OUrandomRoot", parameters = parameters)
  Contr[i] <- sum(dnorm(PossibleContrasts, 0, sqrt(sigma2/alpha), log = TRUE))
}

plot(x = PhyLm, y = Contr, xlim = c(-20, -5), ylim = c(-20, -5), xlab = "PhyloLm LnLik", ylab = "Contrast LnLik")
abline(lm(Contr ~ PhyLm), col = "red")
abline(a = 0, b = 1)


alphas <- seq(0, 4, length.out = 401)
alphas <- alphas[-1]
PossibleContrasts <- sapply(alphas, function(x) getContrasts(phy, dat, x))
loglik_plm <- loglik_con <- vector("numeric", length(alphas))
dat = rTrait(n = 1, phy, model = "OU", parameters = parameters)
for(i in 1:length(alphas)){
  PossibleContrasts <- getContrasts(phy, dat, alphas[i])
  parameters = list(ancestral.state=ancestral.state, alpha=alphas[i],
                    sigma2=sigma2,sigma2_error=sigma2_error,
                    optimal.value=optimal.value)
  loglik_plm[i] <- OU1d.loglik(dat, phy, "OUrandomRoot", parameters = parameters)
  loglik_con[i] <- sum(dnorm(PossibleContrasts, 0, sqrt(sigma2/alphas[i]), log = TRUE))
}


par(mfrow=c(1,2))
plot(x = alphas, y = loglik_plm)
abline(h = max(loglik_plm), col = "red")
abline(v = alphas[which(loglik_plm == max(loglik_plm))], col = "red")
plot(x = alphas, y = loglik_con)
abline(h = max(loglik_con), col = "red")
abline(v = alphas[which(loglik_con == max(loglik_con))], col = "red")
